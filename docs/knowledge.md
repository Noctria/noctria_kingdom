...
- 各AI（設計・実装・テスト・レビュー・ドキュメント）は、knowledge.md・architecture/diagrams/integration_overview.mmd・環境定義を毎ターン参照し、**常に最新ルールを反映**して自己進化する。
...
## ナレッジ/プロンプトの運用ルール
- **knowledge.md, architecture/diagrams/integration_overview.mmd, 各AIプロンプトはバージョン管理の対象とし、重要な変更は必ず履歴・コミットメッセージで明示する。**
