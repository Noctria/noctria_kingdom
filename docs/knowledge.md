...
- テストやビルドで「未定義」「未実装」エラーが発生した場合、AIが自律的に該当クラス・関数・変数等の自動補完プロンプトを生成・投入し、必要な定義・実装を自動生成する補完サイクルを回す。
- この自動補完・修正サイクルで3ターン連続失敗した場合は、人間承認・レビューへ自動遷移し、手動判断となる。
- すべての自動補完実装にも、バージョン・説明責任・型注釈・パス集中管理ルールを厳格に適用すること。
