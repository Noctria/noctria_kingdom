...
- **`path_config.py`は`src/core/`ディレクトリ配下の**ひとつのみ**に厳密に置くこと。他ディレクトリや`generated_code/`配下等に複製・新規生成してはならない。
...
- プロンプトやナレッジもAIが定期的（月次等）に自動読み込みし、運用履歴・エラー傾向・改善案に基づき“自律的にルール進化・最適化”を行うことを推奨。
