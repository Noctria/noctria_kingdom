...
def git_tracked(p):
    return run(f"git ls-files --error-unmatch -- {shlex.quote(p)}").returncode == 0
...
with open(CSV_PATH, newline="", encoding="utf-8") as f:
    ...
    for row in rdr:
        ...
        if not path:
            continue
        if not any(k in cats for k in ("orphaned", "unreferenced", "unused_template")):
            continue
        if cov == "1" or rts == "1":
            continue
        if os.path.basename(path) == "__init__.py":
            print(f"[skip] {path} (__init__.py)")
            continue

        dest = os.path.join(GRAVEYARD, path)
        if os.path.exists(dest):
            print(f"[skip] {path} (already moved)")
            continue
