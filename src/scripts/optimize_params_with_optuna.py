...
def _resolve_env_factory(
    env_target: str, env_kwargs: Optional[dict] = None
...
def run(cfg: OptunaConfig) -> Dict[str, Any]:
    ...
    _log(
        f"🎯 Study: {cfg.study_name} ({cfg.direction}), storage={storage or 'None'}, worker={os.getenv('HOSTNAME', 'worker')}"
    )
    ...
    if cfg.pruner.lower() == "median":
        pruner = optuna_mod.pruners.MedianPruner(
            n_startup_trials=5, n_warmup_steps=cfg.allow_prune_after
        )
    else:
        pruner = None
...
def optimize_main(*args, **kwargs) -> Dict[str, Any]:
    ...
    cfg = OptunaConfig(
        ...
        timeout_sec=(
            int(params.get("timeout_sec")) if params.get("timeout_sec") is not None else None
        ),
