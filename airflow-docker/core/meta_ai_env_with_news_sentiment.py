...
class MetaAIEnv(gym.Env):
    ...
    def reset(self, seed: Optional[int] = None, options: Optional[dict] = None) -> Tuple[np.ndarray, Dict]:
        ...
        self.wins = 0
        self.trades = 0
        return self._get_sentiment_augmented_obs(), {}

    def step(self, action: int) -> Tuple[np.ndarray, float, bool, bool, Dict]:
        self.current_step += 1
        done = self.current_step >= len(self.data) - 1

        obs = self._get_sentiment_augmented_obs()
    ...
    def _get_sentiment_augmented_obs(self) -> np.ndarray:
    ...
    def render(self):
        print(f"🖥️ Current Step: {self.current_step}")
