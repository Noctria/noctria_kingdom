...
def objective(trial):
    ...
    print(
        f"🔮 Prometheus: 学習率 {learning_rate:.6f}、clip_range {clip_range:.2f} で戦略を試します。"
    )
...
def main():
    ...
    print(
        "✅ Prometheus: 最適化完了！見つけた最適な学習率は {:.6f}、clip_rangeは {:.2f}。".format(
            best_params["learning_rate"], best_params["clip_range"]
        )
    )

    # 💾 最適化結果を /opt/airflow/logs/best_params.json に保存
    output_file = "/opt/airflow/logs/best_params.json"
