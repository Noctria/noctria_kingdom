...
def _svg_aspect(svg_path: Path) -> float | None:
    try:
        ...
        for s in ("pt", "px"):
            w = w[: -len(s)] if w.endswith(s) else w
        for s in ("pt", "px"):
            h = h[: -len(s)] if h.endswith(s) else h
...
_EDGE_RE = re.compile(r'(?P<lhs>"[^"]+"|[^\s\[\];]+)\s*->\s*(?P<rhs>"[^"]+"|[^\s\[\];]+)')
...
def _build_layers_from_edges(dot_text: str) -> tuple[list[list[str]], set[tuple[str, str]]]:
    ...
    for m in _EDGE_RE.finditer(dot_text):
        a = _strip_id(m.group("lhs"))
        b = _strip_id(m.group("rhs"))
        if not a or not b:
            continue
        edges.add((a, b))
        nodes.add(a)
...
def build_svg_from_dot() -> bool:
    ...
    if not (size_ok and aspect_ok and nodes_ok):
        print(
            f"! degenerate svg (size_ok={size_ok}, aspect={aspect}, nodes_ok={nodes_ok}) → rebuild without filtering"
        )
        tmp_src = dot_file
        try:
            cmd = [
...
def svg_to_png():
    ...
    try:
        ...
        cairosvg.svg2png(
            url=str(SVG), write_to=str(PNG), output_width=1200, background_color="white"
        )
        tried_cairo = True
...
def _discover_static_roots() -> list[Path]:
    ...
    seen = set()
    return [p for p in roots if not (str(p) in seen or seen.add(str(p)))]
...
def sync_to_static():
    ...
    for sroot in STATIC_ROOTS:
        ...
        if _samepath(dst_root, src_root) or (src_root in dst_root.parents):
            print(f"! skip sync: invalid dst ({dst_root})")
            continue
        for src_dir, _, files in os.walk(GRAPHS):
            sd = Path(src_dir)
            rel = sd.relative_to(GRAPHS)
            for f in files:
                _safe_copy(sd / f, dst_root / "graphs" / rel / f)
        for src_dir, _, files in os.walk(CTXDIR):
            sd = Path(src_dir)
            rel = sd.relative_to(CTXDIR)
            for f in files:
                _safe_copy(sd / f, dst_root / "context" / rel / f)
...
if __name__ == "__main__":
    ...
    if SVG.exists():
        print(
            f"i SVG updated at: {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(SVG.stat().st_mtime))}"
        )
