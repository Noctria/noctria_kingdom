...
def reliability_curve(y_prob: np.ndarray, y_true: np.ndarray, bins: int = 10) -> Tuple[np.ndarray, np.ndarray]:
...
def main():
    ...
    ap.add_argument("--reports", default="reports/veritas", help="run_* 親 or 特定 run ディレクトリ")
    ...
    for run_dir in runs:
        ...
        if not (isinstance(y_prob, list) and isinstance(y_true, list) and len(y_prob) == len(y_true) and len(y_true) >= 5):
