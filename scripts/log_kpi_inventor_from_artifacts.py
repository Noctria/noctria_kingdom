...
def main() -> int:
    ...
    if not root.exists():
        print("[kpi] no strategies dir; skip", file=sys.stderr); return 0

    pattern = os.environ.get("NOCTRIA_STRAT_GLOB", "*")
    dirs = [p for p in root.glob(pattern) if p.is_dir()]
    if not dirs:
        print(f"[kpi] no strategy dirs matched: {pattern}", file=sys.stderr); return 0

    rows, dropped = [], []
    for d in sorted(dirs):
        fv = d / "feature_vector.json"
        if not fv.exists():
            dropped.append(f"{d.name}:no_fv"); continue
        try:
            raw = json.loads(fv.read_text(encoding="utf-8"))
        except Exception:
            dropped.append(f"{d.name}:bad_json"); continue
        vec = _to_flat_float_list(raw)
        if not vec:
            dropped.append(f"{d.name}:not_flat_numeric"); continue
        rows.append((d.name, vec))

    if len(rows) < 2:
        print(f"[kpi] insufficient vectors ({len(rows)}) ; dropped={dropped}", file=sys.stderr); return 0
    ...
    if len(used) < 2:
        print(f"[kpi] vectors filtered by length; still <2; removed={removed}", file=sys.stderr); return 0
    ...
    try:
        ...
        print(f"[kpi] novelty_rate error: {e}", file=sys.stderr); return 0
