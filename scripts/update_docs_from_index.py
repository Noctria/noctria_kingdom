...
def git_log_for_paths(
    ...
    for ln in lines:
        ...
        entries.append(
            CommitEntry(
                sha=sha[:7], iso_time=iso_time, subject=subject, author=author_name, files=files
            )
        )
    return entries
...
def render_git_section(
    title: Optional[str], entries: List[CommitEntry], include_files: bool = True
) -> str:
    lines: List[str] = []
    if title:
        lines += [f"### {title}", ""]
...
def render_file_contents_section(
    ...
    for rel in rel_paths:
        if rel.startswith(":(glob)"):
            ...
            pat = rel[len(":(glob)") :]
            materialized += [
                os.path.relpath(p, start=repo_root)
...
def update_markdown_file(
    ...
    for blk in blocks:
        ...
        if mode == "file_content":
            ...
            section_md = render_file_contents_section(
                repo_root, title=title, rel_paths=paths, fence=fence
            )
        else:
            ...
            section_md = render_git_section(
                title=title, entries=entries, include_files=include_files
            )
    ...
    if new_text != original:
        if dry_run:
            return UpdateResult(
                md_path,
                False,
                f"Would write ({total_changes} block(s) replaced; front-matter {'touched' if fm_before != new_text else 'kept'}).",
            )
        else:
            if create_backup:
            ...
            return UpdateResult(
                md_path,
                True,
                f"Updated ({total_changes} block(s)); front-matter {'updated' if fm_before != new_text else 'unchanged'}.",
            )
    else:
...
def main():
    parser = argparse.ArgumentParser(
        description="Auto-update Markdown docs based on AUTODOC markers."
    )
    parser.add_argument("--docs-root", type=Path, default=Path("docs"))
    parser.add_argument("--repo-root", type=Path, default=Path("."))
    parser.add_argument(
        "--dry-run", type=lambda x: str(x).lower() in ("1", "true", "yes", "y"), default=False
    )
    parser.add_argument(
    ...
    for md in sorted(md_files):
        try:
            res = update_markdown_file(
                ...
                touch_front_matter=args.touch_front_matter,
            )
