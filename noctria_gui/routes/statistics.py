...
from src.core.path_config import GUI_TEMPLATES_DIR, TOOLS_DIR
...
async def show_statistics(request: Request):
    ...
    try:
        ...
        filtered = statistics_service.filter_logs(
            logs=logs, strategy=strategy, symbol=symbol, start_date=start_date, end_date=end_date
        )
        sorted_logs = statistics_service.sort_logs(
            logs=filtered, sort_key="win_rate", descending=True
        )
        stats = statistics_service.get_strategy_statistics()  # ここで集計データを取得
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"統計データの処理中にエラーが発生しました: {e}"
        )
...
async def export_statistics_csv():
    ...
    try:
        logs = statistics_service.load_all_logs()
        sorted_logs = statistics_service.sort_logs(logs=logs, sort_key="win_rate", descending=True)
        if not sorted_logs:
