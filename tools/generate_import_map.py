...
file_map = {
    os.path.splitext(os.path.relpath(f, SRC_ROOT))[0].replace("\\", "/"): f for f in py_files
}
...
isolated_files = [
    k for k in file_map if k not in reverse_imports_map and not k.endswith("__init__")
]
