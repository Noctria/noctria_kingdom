...
def analyze(mmd_path: Path, topk: int = 30) -> Dict[str, dict]:
    ...
    def get_stage_set(nid: str) -> Set[str]:
        return set(
            c
            for c in classes.get(nid, set())
            if c not in ("spine", "gui", "docs", "airflow", "pdca", "plan", "do", "check", "act")
            or c
            in (
                "collect",
                "analyze",
...
def main():
    ...
    # Markdown出力
    lines = [
        "# Workflow Spine (Top {})".format(len(ranked)),
        "",
        "| rank | node | score | fanin | fanout | stage_bridges | flags |",
        "|---:|---|---:|---:|---:|---:|---|",
    ]
    for i, (n, met) in enumerate(
        sorted(ranked.items(), key=lambda kv: kv[1]["score"], reverse=True), start=1
    ):
        lines.append(
            f"| {i} | `{n}` | {met['score']} | {met['fanin']} | {met['fanout']} | {met['stage_bridges']} | {', '.join(met['flags'])} |"
        )
