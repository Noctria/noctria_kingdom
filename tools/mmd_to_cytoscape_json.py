...
def _read_lines(path: Path) -> Iterable[str]:
    ...
    for ln in text.splitlines():
        if RE_CHEAD.match(ln):
            continue
        core = ln.split("%%", 1)[0]  # 行内コメント除去
        if not core.strip():
            continue
        if RE_GRAPH.match(core) or RE_CLASSDEF.match(core):
            continue
        yield core.rstrip()
...
# ====== 解析 ======
def parse_mmd(
    lines: Iterable[str], debug: bool = False, dbg_limit: int = 20
) -> Tuple[Dict[str, Dict], List[Tuple[str, str]], Dict[str, str]]:
    nodes: Dict[str, Dict] = {}
...
def to_cytoscape_json(nodes: Dict[str, Dict], edges: List[Tuple[str, str]], meta: Dict) -> Dict:
    elems_nodes = [
        {"data": {"id": nid, "label": d.get("label", nid), "stage": d.get("stage", "")}}
        for nid, d in nodes.items()
    ]
    elems_edges = [
        {"data": {"id": f"e{i}", "source": s, "target": t}} for i, (s, t) in enumerate(edges)
    ]
...
def main():
    ap = argparse.ArgumentParser(
        description="Convert Mermaid .mmd to Cytoscape.js JSON (with robust edge parsing & debug)"
    )
    ...
    Path(args.output).write_text(
        json.dumps(to_cytoscape_json(nodes, edges, meta), ensure_ascii=False), encoding="utf-8"
    )
