...
OUT = Path("codex_reports/graphs/pages_svg"); OUT.mkdir(parents=True, exist_ok=True)
...
for ln in lines:
    ...
    if m and not m.group(1).startswith("legend_"):
        ...
        for c in m.group(2).split(","):
            c = c.strip()
            if c in {"collect","analyze","plan","backtest","forwardtest",
                     "fintokei_check","mt5_order","settlement",
                     "result_analyze","merge_info","replanning"}:
...
for st, mem in classes.items():
    ...
    sub.write_text("\n".join(out_lines) + "\n", encoding="utf-8")
    subprocess.run([
        "mmdc","-i",str(sub),"-o",str(OUT/f"{st}.svg"),
