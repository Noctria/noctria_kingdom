...
# 除外パス（生成物や墓場など）
IGNORE_DIRS = {
    ".git", ".ruff_cache", ".mypy_cache", ".venv", "venv", "__pycache__",
    "actions-runner", "_graveyard", "generated_code", "data/models",
    ".ipynb_checkpoints",
}
...
def guess_stage(p: Path, content: str) -> Tuple[Optional[str], List[str]]:
    ...
    for stage, pats in STAGES:
        reason = matches_any(pats, path_text) or matches_any(pats, fname) or matches_any(pats, content[:2000])
        if reason:
