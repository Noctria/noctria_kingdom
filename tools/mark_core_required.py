...
def resolve_targets(
    items: Iterable[str], project_root: Path, nodeidx: Dict[str, Path]
...
def insert_mark(p: Path, dry_run: bool = False) -> str:
    if file_already_marked(p):
        return f"[ok] already marked: {p}"
    if dry_run:
        return f"[dry] would mark: {p}"
    try:
...
def main():
    ...
    if not keep_file.exists():
        print(f"[fatal] keep_list not found: {keep_file}")
        raise SystemExit(2)
