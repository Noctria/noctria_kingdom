...
class RiskControl:
    ...
    def __init__(
        self, initial_capital: float, daily_loss_pct: float = 0.05, total_loss_pct: float = 0.10
    ):
        # --- 基本設定 ---
...
if __name__ == "__main__":
    ...
    is_ok, reason = rc.pre_trade_check(
        new_trade_size=1.0, current_time=datetime.datetime.now() + datetime.timedelta(minutes=1)
    )
    print(f"日次損失超過チェック: OK? {is_ok}, 理由: {reason}")

    # マーチンゲール違反のチェック
    is_ok, reason = rc.pre_trade_check(
        new_trade_size=1.1, current_time=datetime.datetime.now() + datetime.timedelta(minutes=2)
    )
