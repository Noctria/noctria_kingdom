...
def detect_timeseries_anomalies(
    ...
    if columns is None:
        ...
        columns = [
            ...
            if any(
                w in c.lower()
                for w in ["win_rate", "drawdown", "dd", "news", "cpi", "volatility", "spike"]
            )
        ]
    for col in columns:
...
def detect_event_shock_days(
    df: pd.DataFrame, event_col: str, target_col: str, thresh: float = 2.0
) -> pd.DataFrame:
    """
    ...
    anomaly_days = ev_days[
        (ev_days[target_col] > mean + std * thresh) | (ev_days[target_col] < mean - std * thresh)
    ]
    return anomaly_days
...
def summarize_anomalies(
    eval_results: List[Dict], df: Optional[pd.DataFrame] = None
) -> Dict[str, Any]:
    """
    検出異常をまとめて要約
    """
